...
void cCallback(IPC_Message* msg){
  ...
  ConnectionCallback cb = (ConnectionCallback) ht_get(ht, name);
...
IPC::Connection::Connection(char* name, int type, int create=1){
  if (create){
...
void IPC::Connection::setCallback(ConnectionCallback cb){
  ht_put(ht, ((IPC_Connection*) ptr)->name, cb);
  connectionSetCallback((IPC_Connection*) ptr, cCallback);
}

ConnectionCallback IPC::Connection::getCallback(){
  return (ConnectionCallback) ht_get(ht, ((IPC_Connection*) ptr)->name);
}
...
void IPC::Connection::connectionRemoveSubscription(char* subject){
